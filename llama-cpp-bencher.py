...
def write_summary(df,out):
    ...
    for (b,fa,bf,hiplt),grp in df.groupby(['build','fa','b','hipblaslt']):
        ...
        pp512=_get('pp',512)
        tg128=_get('tg',128)
        vram=grp['vram_delta_mib'] if 'vram_delta_mib' in grp.columns else 0
        gtt=grp['gtt_delta_mib'] if 'gtt_delta_mib' in grp.columns else 0
        mem=(vram+gtt).max() if hasattr((vram+gtt),'max') else 0
